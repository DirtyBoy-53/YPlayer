...
std::atomic_flag YFFPlayer::s_ffmpeg_init = ATOMIC_FLAG_INIT;
...
static int interrupt_callback(void* opaque) {
    ...
    YFFPlayer* player = (YFFPlayer*)opaque;
    if (player->quit ||
...
bool YFFPlayer::doFinish() {
    int ret = close();
    event_callback(HPLAYER_CLOSED);
    return ret == 0;
}

void YFFPlayer::doTask() {
